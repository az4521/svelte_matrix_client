<script lang="ts">
	import { favouritesState, removeFavouriteGif, type FavouriteGif } from '$lib/stores/favourites.svelte';
	...
	let { onSelect, onClose, onSwitchToEmoji, onSwitchToSticker }: Props = $props();
	...
	function onKeydown(e: KeyboardEvent) {
		...
		if (e.ctrlKey && e.key === 'g') { e.preventDefault(); onClose(); }
	}

	function onSearchKeydown(e: KeyboardEvent) {
		if (e.key === 'ArrowDown') {
			e.preventDefault();
			if (selectedIndex === -1) { if (visibleGifs.length > 0) selectedIndex = 0; }
			else selectedIndex = Math.min(selectedIndex + COLS, visibleGifs.length - 1);
	...
	<div class="flex-1 overflow-y-auto min-h-0 px-2 pb-2">
		{#if visibleGifs.length === 0}
			<p class="text-center text-discord-textMuted text-sm py-8 px-4">
				{gifs.length === 0 ? 'No favourite GIFs yet. Star a GIF in chat to save it here.' : 'No results'}
			</p>
		{:else}
