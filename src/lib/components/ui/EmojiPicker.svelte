<script lang="ts">
	...
	let { onSelect, onSelectCustom, onClose, onSwitchToSticker, onSwitchToGif }: Props = $props();
	...
	const tabs = $derived([
		...
		...EMOJI_CATEGORIES.map((c) => ({ id: c.id, label: c.label, avatarUrl: undefined, isCustom: false as const }))
	]);
	...
	const searchCustom = $derived(
		search
			? customPacks.flatMap((p) =>
					p.emojis
						.filter((e) => e.shortcode.toLowerCase().includes(search.toLowerCase()))
						.map((e) => ({ ...e, packId: p.id }))
				)
			: []
	);
	const searchStandard = $derived(
		search ? ALL_EMOJIS.filter((e) => e.name.includes(search.toLowerCase())) : []
	);
	...
	type FlatItem =
		| { kind: 'custom'; sectionId: string; data: CustomEmoji & { packId: string } }
		| { kind: 'standard'; sectionId: string; data: { emoji: string; name: string } };

	const flatItems = $derived.by((): FlatItem[] => {
		if (search) {
			return [
				...searchCustom.map((e): FlatItem => ({ kind: 'custom', sectionId: 'search-custom', data: e })),
				...searchStandard.map((e): FlatItem => ({ kind: 'standard', sectionId: 'search-standard', data: e })),
			];
		}
		const items: FlatItem[] = [];
		for (const pack of customPacks) {
			for (const e of pack.emojis) {
				items.push({ kind: 'custom', sectionId: pack.id, data: { ...e, packId: pack.id } });
			}
		}
	...
	$effect(() => {
		...
		if (!revealedSections.has(item.sectionId) &&
	...
	function onKeydown(e: KeyboardEvent) {
		...
		if (e.ctrlKey && e.key === 'e') { e.preventDefault(); onClose(); }
		if (e.ctrlKey && e.key === 's') { e.preventDefault(); onSwitchToSticker?.(); }
	...
	function stdPlaceholderHeight(count: number) { return Math.ceil(count / 6) * 44; }
	function customPlaceholderHeight(count: number) { return Math.ceil(count / 6) * 44; }
</script>
	...
	<div bind:this={scrollEl} onscroll={onScroll} class="relative flex-1 overflow-y-auto min-h-0 px-2 pb-2">
		{#if search}
			...
			{#if searchCustom.length > 0}
				<p class="text-xs font-semibold text-discord-textMuted uppercase tracking-wide px-1 py-1">Custom</p>
				<div class="grid grid-cols-6 gap-1 mb-2">
					{#each searchCustom as e, li (e.packId + ':' + e.shortcode)}
						{@const globalIdx = (sectionOffsets.get('search-custom') ?? 0) + li}
						<button
			...
			{#if searchStandard.length > 0}
				{#if searchCustom.length > 0}
					<p class="text-xs font-semibold text-discord-textMuted uppercase tracking-wide px-1 py-1">Standard</p>
				{/if}
				<div class="grid grid-cols-6 gap-1">
					{#each searchStandard as e, li (e.name)}
						{@const globalIdx = (sectionOffsets.get('search-standard') ?? 0) + li}
						<button
							data-item-index={globalIdx}
		...
		{:else}
			...
			{#each customPacks as pack (pack.id)}
				<div data-lazy-id={pack.id} use:lazySection>
					...
					{:else}
						<div class="mb-2" style="height: {customPlaceholderHeight(pack.emojis.length)}px"></div>
					{/if}
				</div>
			{/each}
			...
			{#each EMOJI_CATEGORIES as cat (cat.id)}
				<div data-lazy-id={cat.id} use:lazySection>
					...
					{#if revealedSections.has(cat.id)}
						<div class="grid grid-cols-6 gap-1 mb-2">
							{#each cat.emojis as e, li (e.name)}
								{@const globalIdx = (sectionOffsets.get(cat.id) ?? 0) + li}
								<button
									data-item-index={globalIdx}
									onclick={() => pick(e.emoji)}
									title={e.name}
					...
					{:else}
						<div class="mb-2" style="height: {stdPlaceholderHeight(cat.emojis.length)}px"></div>
					{/if}
				</div>
			{/each}
		{/if}
