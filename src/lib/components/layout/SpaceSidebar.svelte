  ...
  function performDrop() {
    ...
    if (!dropTarget) {
      ...
      if (dragState.fromFolderId !== null) {
        ...
        layout.folders[dragState.fromFolderId].spaceIds = layout.folders[
          dragState.fromFolderId
        ].spaceIds.filter((id) => id !== dragState!.id);
      ...
      <button
        ...
        ontouchstart={(e) =>
          onItemTouchStart(e, item.space.roomId, null, (x, y) =>
            openSpaceContextMenu({ clientX: x, clientY: y }, item.space.roomId),
          )}
        ...
        <button
          ...
          ontouchstart={(e) =>
            onItemTouchStart(e, item.id, null, (x, y) =>
              openFolderContextMenu({ clientX: x, clientY: y }, item.id),
            )}
          ontouchmove={onItemTouchMove}
          ontouchend={onItemTouchEnd}
        ...
        >
          <div
            ...
            style="display: grid; grid-template-columns: repeat({Math.min(
              item.spaces.length,
              2,
            )}, 1fr); grid-template-rows: repeat({item.spaces.length > 2
              ? 2
