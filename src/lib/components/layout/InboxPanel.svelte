<script lang="ts">
	...
	import { getRoomAvatar, getRoomDisplayName, getRoomTopic, acceptInvite, rejectInvite, getInviteSender } from '$lib/matrix/client';
	...
	<div class="flex-1 overflow-y-auto p-4">
		{#if roomsState.invitedRooms.length === 0}
			<div class="flex flex-col items-center justify-center h-full text-center py-16">
				<div class="w-16 h-16 rounded-full bg-discord-backgroundSecondary flex items-center justify-center mb-4">
					<svg class="w-8 h-8 text-discord-textMuted" fill="currentColor" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4-8 5-8-5V6l8 5 8-5v2z"/></svg>
				</div>
				<p class="text-discord-textPrimary font-semibold mb-1">No pending invites</p>
				<p class="text-discord-textMuted text-sm">Room invites will appear here.</p>
			</div>
		{:else}
			<div class="max-w-lg mx-auto flex flex-col gap-3">
				<p class="text-xs font-semibold text-discord-textMuted uppercase tracking-wide mb-1">Pending invites — {roomsState.invitedRooms.length}</p>
				{#each roomsState.invitedRooms as room (room.roomId)}
					...
					<div class="flex items-center gap-4 p-4 rounded-lg bg-discord-backgroundSecondary border border-discord-divider">
						<Avatar src={getRoomAvatar(room)} name={getRoomDisplayName(room)} size={48} />
						<div class="flex-1 min-w-0">
							<p class="font-semibold text-discord-textPrimary truncate">{getRoomDisplayName(room)}</p>
							{#if sender}
								<p class="text-xs text-discord-textMuted">Invited by {sender}</p>
							{/if}
							{#if topic}
								<p class="text-sm text-discord-textMuted mt-0.5 line-clamp-2">{topic}</p>
							{/if}
						</div>
