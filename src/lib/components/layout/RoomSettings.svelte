<script lang="ts">
  ...
  async function saveAccess() {
    ...
    try {
      ...
      if (historyVisibility !== getHistoryVisibility(room))
        promises.push(setHistoryVisibility(room.roomId, historyVisibility));
      await Promise.all(promises);
      accessSuccess = true;
  ...
  const filteredMembers = $derived(
    allMembers
      ...
      .sort(
        (a, b) => b.powerLevel - a.powerLevel || a.name.localeCompare(b.name),
      ),
  );
      ...
      <div class="flex-1 overflow-y-auto p-6 min-w-0">
        ...
        {#if activeTab === "general"}
          <div class="space-y-5">
              ...
              <div class="flex items-center gap-4">
                ...
                >
                  {#if currentAvatarUrl}
                    ...
                    />
                  {:else}
                    <span class="text-2xl font-bold text-discord-textMuted"
                      >{room.name?.[0]?.toUpperCase() ?? "#"}</span
                    >
                  {/if}
                </div>
                {#if canEditState}
            ...
            {#if generalError}<p class="text-sm text-discord-danger">
                {generalError}
              </p>{/if}
            {#if canEditState}
              <button
            ...
            {#each [["Send messages", "plEventsDefault"], ["Change room settings", "plStateDefault"], ["Default member level", "plUsersDefault"], ["Invite members", "plInvite"], ["Kick members", "plKick"], ["Ban members", "plBan"], ["Redact messages", "plRedact"]] as [label, key]}
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2">
                  ...
                  />
                  <span class="text-xs text-discord-textMuted w-16"
            ...
            {#if showBanned}
              ...
              <div class="space-y-1">
                {#each bannedMembers as member (member.userId)}
                  ...
                  >
                    <Avatar
                      src={mxcToHttp(member.getMxcAvatarUrl())}
                      name={member.name}
                      size={28}
                    />
              ...
              <div class="space-y-1">
                {#each filteredMembers as member (member.userId)}
                  ...
                  {@const canActOnMember =
                    !isSelf && myPowerLevel > member.powerLevel}
                  <div
                  ...
                  >
                    <div class="flex items-center gap-3 p-2">
                      ...
                      <span class="text-xs text-discord-textMuted flex-shrink-0"
                        ...
                        <div class="flex flex-wrap gap-2">
                          {#if myPowerLevel >= 100 || myPowerLevel > member.powerLevel}
                            ...
                            >
                              ...
                              {#if myPowerLevel >= 100}<option value="100"
                                  >Admin (100)</option
                                >{/if}
                              {#if myPowerLevel >= 50}<option value="50"
                                  >Moderator (50)</option
                                >{/if}
          ...
          <div class="space-y-3">
            <p class="text-xs text-discord-textMuted">
              Set the <code
                ...
                >order</code
              > field on each child room to control sort order (lexicographic). Leave
              blank to sort by creation time.
            </p>
                  ...
                  {#if canEditState}
                    ...
                    <button
                      ...
                      disabled={roomActionPending === child.roomId ||
                        (orderEdits[child.roomId] ?? "") === child.order}
