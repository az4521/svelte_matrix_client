<script lang="ts">
	...
	// Context menu state
	let contextMenu = $state<{ roomId: string; x: number; y: number } | null>(
		null,
	);
...
<div class="w-60 bg-discord-backgroundSecondary flex flex-col flex-shrink-0">
	...
	>
		...
		{#if canAccessSpaceSettings}
			<button
				onclick={() =>
					activeSpaceRoom && onOpenSpaceSettings?.(activeSpaceRoom)}
		...
		{#if visibleRooms.length > 0}
			<div class="mb-2">
				...
				{#each visibleRooms as room (room.roomId)}
					...
					>
						<span
							class="w-5 h-5 flex-shrink-0 opacity-70 font-semibold flex items-center justify-center"
							>#</span
						>
