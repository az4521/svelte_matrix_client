<script lang="ts">
  ...
  $effect(() => {
    ...
    Promise.all(
      ids.map(async (id) => {
        return (
          findEventById(room, id) ?? (await fetchEventById(room.roomId, id))
        );
      }),
    ).then((results) => {
          ...
          >
            <div class="flex items-center gap-2 mb-1">
              ...
              <span
                ...
                >{name}</span
              >
              <span class="text-xs text-discord-textMuted ml-auto flex-shrink-0"
                >{format(event.getTs(), "MMM d")}</span
              >
            </div>
            <p class="text-xs text-discord-textMuted line-clamp-3 break-words">
              {excerpt(event)}
            </p>
            <div
            ...
            >
              <button
                ...
                class="text-xs text-discord-accent hover:underline">Jump</button
              >
              {#if canPin}
