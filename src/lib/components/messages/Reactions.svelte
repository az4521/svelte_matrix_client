<script lang="ts">
	...
	$effect(() => {
		const serverEventIds = new Set(reactions.flatMap((g) => (g.myEventId ? [g.myEventId] : [])));
		for (const id of pendingRemovals) {
			if (!serverEventIds.has(id)) {
				pendingRemovals = new Set([...pendingRemovals].filter((i) => i !== id));
			}
		}
	});
		...
		reactions
			.map((g) => {
				if (g.myEventId && pendingRemovals.has(g.myEventId)) {
					return { ...g, count: g.count - 1, isMine: false, myEventId: null };
				}
				return g;
			})
			.filter((g) => g.count > 0)
	...
	async function toggleReaction(key: string, isMine: boolean, myEventId: string | null) {
		if (isMine) {
			...
			try {
				...
				// Restore on failure
				pendingRemovals = new Set([...pendingRemovals].filter((id) => id !== myEventId));
...
{#if visibleReactions.length > 0}
	<div class="flex flex-wrap gap-1 mt-1">
		{#each visibleReactions as group (group.key)}
			{@const imgSrc = group.key.startsWith('mxc://') ? mxcToHttp(group.key, 32) : null}
			<button
			...
			>
				{#if imgSrc}
					<img src={imgSrc} alt={group.key} class="w-4 h-4 object-contain" />
