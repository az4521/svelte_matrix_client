      ...
      >
        ...
        <div class="flex items-center gap-2">
          <span class="font-mono text-xs text-gray-400"
            >Sync: <span class="text-green-400">{syncState}</span></span
          >
          <button
            onclick={refresh}
            ...
            <div class="px-3 py-1 border-b border-[#222] hover:bg-[#1f1f3a]">
              <div class="flex gap-2 items-baseline">
                ...
                <span
                  ...
                  class:bg-green-900={s.status === "confirmed"}
                  class:bg-yellow-900={s.status !== "confirmed"}
                >
                  {s.status}
                </span>
                {#if s.isRedacted}<span class="text-red-400">REDACTED</span
                  >{/if}
          ...
          {#each pendingEvents as e, i}
            ...
            <div class="px-3 py-1 border-b border-[#222] hover:bg-[#1f1f3a]">
              <div class="flex gap-2 items-baseline flex-wrap">
                ...
                <span class="text-blue-300">{s.type}</span>
                <span class="text-purple-300">{s.sender.split(":")[0]}</span>
                <span class="px-1 rounded text-[10px] bg-yellow-900"
                  >{s.status}</span
                >
                {#if s.relType}<span class="text-orange-400"
                    >rel={s.relType}</span
          ...
          {#each allTimelineEvents as e, i}
            ...
            <div class="px-3 py-1 border-b border-[#1a1a1a] hover:bg-[#1f1f3a]">
              <div class="flex gap-2 items-baseline flex-wrap">
                ...
                <span class="text-yellow-300">{formatTs(s.ts)}</span>
                <span
                  class:text-blue-300={s.type === "m.room.message"}
                  class:text-gray-500={s.type !== "m.room.message"}
                >
                  {s.type}
                </span>
                <span class="text-purple-300">{s.sender.split(":")[0]}</span>
