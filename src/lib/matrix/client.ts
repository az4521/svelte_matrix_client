...
export async function fetchSpaceHierarchy(
  ...
  try {
    ...
    const result = (await (matrixClient as unknown as Record<string, Function>)[
      "getRoomHierarchy"
    ](spaceId, 200, 1)) as { rooms: Array<Record<string, unknown>> };
    ...
    if (spaceEntry) {
      const childrenState =
        (spaceEntry["children_state"] as Array<Record<string, unknown>>) ?? [];
      for (const ev of childrenState) {
        if (ev["type"] === "m.space.child") {
          const childRoomId = ev["state_key"] as string;
          const via =
            ((ev["content"] as Record<string, unknown>)?.["via"] as string[]) ??
            [];
...
export function getSpaceLayout(): SpaceLayout {
  ...
  const layout = matrixClient.getAccountData(SPACE_LAYOUT_KEY)?.getContent() as
    | SpaceLayout
    | undefined;
  ...
  try {
    ...
    for (const e of relations.getRelations()) {
      ...
      groups.set(key, {
        ...
        myEventId:
          isOwn && !e.status ? (e.getId() ?? null) : existing.myEventId,
      });
    }
...
export function getJoinRule(room: Room): string {
  const state = room.getLiveTimeline().getState(EventTimeline.FORWARDS);
  return (
    state?.getStateEvents("m.room.join_rules", "")?.getContent()?.join_rule ??
    "invite"
  );
}
